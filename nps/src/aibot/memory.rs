...
impl SqliteMemory {
    ...
    async fn get_or_compute_embedding(&self, text: &str) -> anyhow::Result<Option<Vec<f32>>> {
...

        {
            ...
            if let Some(bytes) = cached {
                ...
                return Ok(Some(utils::bytes_to_vec(&bytes)));
            }
        }

        // Compute embedding
        let embedding = self.embedder.embed_one(text).await?;
        let bytes = utils::vec_to_bytes(&embedding);
    ...
    fn vector_search(
        ...
        for row in rows {
            let (id, blob) = row?;
            let emb = utils::bytes_to_vec(&blob);
            let sim = utils::cosine_similarity(query_embedding, &emb);
            if sim > 0.0 {
                scored.push((id, sim));
            }
        }
    ...
    pub async fn reindex(&self) -> anyhow::Result<usize> {
        ...
        for (id, content) in &entries {
            if let Ok(Some(emb)) = self.get_or_compute_embedding(content).await {
                let bytes = utils::vec_to_bytes(&emb);
                let conn = self
                    .conn
                    .lock()
                    .map_err(|e| anyhow::anyhow!("Lock error: {e}"))?;
...
impl Memory for SqliteMemory {
    ...
    async fn store(
        ...
        let embedding_bytes = self
            ...
            .await?
            .map(|emb| utils::vec_to_bytes(&emb));
